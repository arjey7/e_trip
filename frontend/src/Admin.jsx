...
function Admin() {
    ...
    const handleSubmit = async (e) => {
        ...
        if (formData.photo) {
            ...
            const photoResponse = await fetch('http://localhost:1111/api/files/img', {
                method: 'POST',
                body: photoData
            });
        ...
        if (formData.video) {
            ...
            const videoResponse = await fetch('http://localhost:1111/api/files/video', {
                method: 'POST',
                body: videoData
            });
                ...
                <table className="table">
                    ...
                    {tours.map((tour, index) => (
                        <tr className={"op"} onClick={() => selectedTour(tour.id)} key={index}>
                            ...
                            <td>{tour.text}</td>
                            <td><img src={`http://localhost:1111/api/files/img?name=${tour.photo}`} alt="Tour"/></td>
                            <td>
                                <video width="320" height="240" controls>
                                    <source src={`http://localhost:1111/api/files/video?name=${tour.video}`} type="video/mp4"/>
